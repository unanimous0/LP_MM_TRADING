...
## [Status]
- **현재 작업**: v6.1 — UI 추가 정리 + recent 안정화 + 단기 정렬 제거
- **마지막 업데이트**: 2026-03-31
...
- **점수 공식 v2**: 6개→4개 구성요소 단순화 + supply_persistence(수급 지속 강도) 추가
  - 수급강도(recent=5D×0.3+20D×0.7) ×0.35 + 수급추세(long_divergence) ×0.20 + 종합수급(weighted) ×0.25 + 수급지속(supply_persistence) ×0.20
...
└─ 패턴: 급등형/지속형/전환형 (순매도 패턴)
   └─ 숏 전략: 공매도 또는 주식선물 매도
```

**주요 변경사항**:
1. **패턴 이름은 동일** (급등형/지속형/전환형)
   - 순매수 급등형: 단기 급등 (양수 Z-Score, recent > 0.5, momentum > 1.0)
   - 순매도 급등형: 단기 급락 (음수 Z-Score, recent < -0.5, momentum < -1.0)
...
### 2026-03-25~31 (v6.1 — UI 추가 정리 + recent 안정화)

**UI 정리**:
- 종목탐색 점수순위 탭 제거 (메인과 중복) → 3탭: 누적수급/섹터분석/패턴가이드
- 메인 탭명: "수급 TOP" → "중단기 수급", "이상수급" → "오늘 이상수급"
- 단기 기준 정렬 라디오 추가 후 제거 (종합점수와 거의 동일 결과로 중복)
- 메인 드릴다운에 관심종목 추가/해제 버튼 추가
- 주가 차트 수급 오버레이: 단일 선택 → multiselect(외국인/기관/개인 중복 선택)
- 주가 차트 좌축(순매수 억원) / 우축(주가 원) 분리 + Y축 소수점 제거
- X축 날짜 포맷: ko-KR yyyy-MM-dd
- 사이드바 기간 라벨 명확화: "점수 보정 표본 기간" / "누적 집계 기간"
- 사이드바 순서: 필터 먼저 → 고급설정 하단
- 시총 필터 기본값: 전체 → 500위 이내
- 표시 종목 수 기본값: 20 → 50, 테이블 높이: 20행

**recent 공식 안정화**:
- `(5D + 20D) / 2` → `5D×0.3 + 20D×0.7`
- 5D가 전체 점수에 미치는 영향: 17.5% → 10.5%
- 5일 윈도우 구성 변경으로 인한 일별 순위 변동 완화

**DB 데이터 이슈 발견**:
- 2026-03-04 이후 삼성전자 등 순매수 금액이 ~1000배 작아짐 (단위 변경 의심)
- 공유 DB 크롤러 쪽 확인 필요

**누락 데이터 확인**:
- `index_components` 테이블: 구조 있으나 데이터 0건 (지수 구성종목)
- `stocks` 테이블: 환기주의/관리종목 상태 컬럼 없음
- 공유 DB 프로젝트에서 수집 추가 필요
...
**원인**: 두 거래일 사이를 선형 보간하여 교차점 x좌표를 계산 → 금요일~월요일 사이 토/일 날짜가 산출됨

**수정 내용**:
- ✅ **보간 x좌표(시각 위치) 유지**: Plotly 연속 시간축에서 삼각형이 MA선 교차점에 정확히 위치
- ✅ **호버 날짜만 거래일로 표시**: `customdata`에 `c_date`(교차 확인된 거래일) 저장 → `hovertemplate`에서 `%{customdata}` 사용
- `golden_hover`/`dead_hover` 리스트 추가

**파일** (1개):
```
app/utils/charts.py    (create_signal_ma_chart 골든/데드크로스 호버 날짜 수정)
```
...
**프로젝트 버전**: v6.1.0 (recent 안정화 + UI 추가 정리 + 중단기수급 단일 정렬)
**마지막 업데이트**: 2026-03-31
