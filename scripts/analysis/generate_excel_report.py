...
def create_glossary_sheet(writer, signal_bonus: int):
    ...
    glossary_data = [
        ...
        ['', '', ''],
        ['■ 보고서 개요', '', ''],
        ['분석 목적', '외국인/기관 투자자 수급 흐름을 정량화하여 매수 강도가 높은 종목을 발굴합니다.', ''],
        ['분석 기간', '1주(1W) ~ 2년(2Y)까지 6개 기간을 분석하여 단기/중기/장기 트렌드를 종합합니다.', ''],
        ['데이터 출처', 'KOSPI200 + KOSDAQ150 총 345개 핵심 종목 (2024-01-02 ~ 2026-01-20)', ''],
        ['', '', ''],
        ['■ 핵심 지표 정의', '', ''],
        ['', '', ''],
        ['📌 Sff (Supply Float Factor)', '', ''],
        ['정의', 'Sff = (순매수 금액 / 유통시총) × 100', ''],
        ['의미', '시가총액 왜곡을 제거하고 유통물량 대비 매수 강도를 정규화한 지표입니다.', ''],
        ['해석', '값이 클수록 유통물량 대비 순매수 금액이 크다는 의미입니다.', ''],
        ['', '', ''],
        ['📌 Z-Score', '', ''],
        ['정의', 'Z-Score = (현재값 - 60일 평균) / 60일 표준편차', ''],
        ['의미', '변동성을 보정하여 이상 수급(평소와 다른 매수/매도)을 탐지합니다.', ''],
        ['해석', '|Z| > 2.0: 이상 수급 발생, +값: 평소보다 강한 매수, -값: 평소보다 강한 매도', ''],
        ['', '', ''],
        ['■ 패턴 분류 (3가지 유형)', '', ''],
        ['', '', ''],
        ...
        ['투자 스타일', '단기 트레이딩, 돌파 매매에 적합', ''],
        ['위험도', '높음 (변동성 큼, 손절 엄격 필요)', ''],
        ['', '', ''],
        ['📈 지속형 (Sustained Pattern)', '', ''],
        ['특징', '장기간 일관된 상승 추세를 보이는 종목', ''],
        ['조건', '가중평균 > 0.8 AND 양수 기간 비율 > 70%', ''],
        ['투자 스타일', '중장기 추세 추종, 포지션 트레이딩에 적합', ''],
        ['위험도', '중간 (안정적 상승, 장기 보유 가능)', ''],
        ['', '', ''],
        ['🔄 전환형 (Reversal Pattern)', '', ''],
        ['특징', '과거 강했으나 최근 약화 → 반대 방향 전환 대기 종목', ''],
        ['조건', '가중평균 > 0.5 AND 1W-2Y < 0', ''],
        ['투자 스타일', '저가 매수 기회 포착, 역추세 매매', ''],
        ['위험도', '높음 (추세 전환 실패 가능성, 신중한 진입 필요)', ''],
        ['', '', ''],
        ['■ 시그널 (3가지 타이밍 지표)', '', ''],
        ['', '', ''],
        ['✅ MA 골든크로스', '', ''],
        ['정의', '외국인 순매수 5일MA > 20일MA 돌파', ''],
        ['의미', '단기 수급이 장기 추세를 상향 돌파 → 매수 타이밍', ''],
        ['', '', ''],
        ['⚡ 수급 가속도', '', ''],
        ['정의', '(최근 5일 평균 순매수) / (직전 5일 평균 순매수) > 1.5배', ''],
        ['의미', '수급 강도가 급격히 증가 → 모멘텀 가속', ''],
        ['', '', ''],
        ['🤝 외인-기관 동조율', '', ''],
        ['정의', '최근 20일 중 외국인과 기관이 함께 매수한 날의 비율 > 50%', ''],
        ['의미', '두 투자 주체가 동시 매수 → 확신도 높음', ''],
        ['', '', ''],
        ['■ 점수 메트릭 (4가지 정렬 기준)', '', ''],
        ['', '', ''],
        ['📍 Recent (현재 강도)', '', ''],
        ['계산식', '(1W + 1M) / 2', ''],
        ['의미', '최근 1주~1개월 수급 강도의 평균', ''],
        ['활용', '현재 진행형 매수세 파악', ''],
        ['', '', ''],
        ['📍 Momentum (수급 개선도)', '', ''],
        ['계산식', '1W - 2Y', ''],
        ['의미', '단기(1주) vs 장기(2년) 수급 격차 → 전환점 포착', ''],
        ['활용', '과거 대비 수급 개선 여부 판단', ''],
        ['', '', ''],
        ['📍 Weighted (가중 평균)', '', ''],
        ['계산식', '1W×0.30 + 1M×0.25 + 3M×0.20 + 6M×0.15 + 1Y×0.07 + 2Y×0.03', ''],
        ['의미', '최근 기간에 높은 가중치를 부여한 중장기 트렌드', ''],
        ['활용', '중장기 추세 방향 판단', ''],
        ['', '', ''],
        ['📍 Average (일관성)', '', ''],
        ['계산식', '(1W + 1M + 3M + 6M + 1Y + 2Y) / 6', ''],
        ['의미', '전체 기간의 단순 평균 → 일관된 수급 파악', ''],
        ['활용', '전 기간 고르게 강한 종목 발굴 (deprecated)', ''],
        ['', '', ''],
        ['■ 종합점수 & 추천 기준', '', ''],
        ['', '', ''],
        ['종합점수 계산', f'패턴 점수 + (시그널 개수 × {signal_bonus}점)', ''],
    ...
    # DataFrame으로 변환
    df_glossary = pd.DataFrame(glossary_data, columns=['항목', '내용', '비고'])

    # 시트에 쓰기
    df_glossary.to_excel(writer, sheet_name='0.용어설명', index=False)
    ...
    if HAS_OPENPYXL:
        ...
        for row in ws.iter_rows():
            for cell in row:
                ...
                # 섹션 헤더 (■ 포함) 강조
                if cell.value and '■' in str(cell.value):
                    cell.font = Font(name='맑은 고딕', size=11, bold=True, color='1F4E78')
