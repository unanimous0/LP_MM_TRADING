...
**원인**: 두 거래일 사이 선형 보간으로 교차점 x좌표 계산 → 금요일~월요일 사이 토/일 날짜 산출
**수정**: 보간 x좌표(시각 위치) 유지 + `customdata`로 거래일(c_date) 호버 표시
**파일**: `app/utils/charts.py` (create_signal_ma_chart)
...
- ✅ **B-1: 관심종목 저장** (`data_loader.py` + 3개 페이지)
  - SQLite `watchlist` 테이블 (stock_code, stock_name, sector, added_at, note)
  - `add_to_watchlist()`, `remove_from_watchlist()`, `is_in_watchlist()`, `get_watchlist()` CRUD
  - `5_📋_종목상세.py`: 헤더 옆 ⭐ 관심 추가/해제 토글 버튼
  - `2_🔍_패턴분석.py`: 종목 리스트 탭에 ⭐ 컬럼 + "관심종목 관리" expander (멀티셀렉트 추가/제거)
  - `streamlit_app.py`: KPI 저장 관심종목 수 표시 + "저장된 관심종목" 탭 (분석 결과 조인, 제거 UI)

- ✅ **B-4: 백테스트 결과 히스토리** (`data_loader.py` + `3_📈_백테스트.py`)
  - SQLite `backtest_history` 테이블 (run_at, strategy, 기간, 수익률, MDD, 샤프, 칼마, 파라미터 등)
  - `save_backtest_history(result, start_date, end_date, note, label)` → PerformanceMetrics 자동 계산
  - `get_backtest_history(limit=50)`, `delete_backtest_history(id)` CRUD
  - 백테스트 페이지 하단: "💾 이 결과를 히스토리에 저장" expander (레이블/메모 입력)
  - "📋 백테스트 히스토리" expander: 이전 실행 비교 테이블 + 항목 삭제 UI

- ✅ **B-3: 고득점 변동 알림** (`data_loader.py` + `streamlit_app.py`)
  - SQLite `score_change_log` 테이블 (analysis_date, stock_code, score, prev_score, change_type)
  - `snapshot_scores(report_df, analysis_date)`: 직전 스냅샷과 비교 → new_entry/score_up/score_down/exit 기록
  - `get_score_change_alerts(limit=100)`: 알림 조회
  - 홈 페이지 하단: 🔔 고득점 변동 알림 섹션 (이벤트 유형 필터 + 테이블)
  - 홈 페이지 방문 시 자동 스냅샷 (세션당 1회)

- ✅ **B-2: 종목 비교 페이지** (`6_🔀_종목비교.py` 신규 + `charts.py` 3개 함수)
  - 사이드바: 최대 5종목 멀티셀렉트 (관심종목 기본 선택), 기준일/기간/Z-Score기간
  - 4탭 구조:
...
**새 DB 테이블 3개** (모두 `IF NOT EXISTS`, 기존 DB에 자동 추가):
```
watchlist           — 관심종목 (stock_code PK)
backtest_history    — 백테스트 히스토리 (autoincrement ID)
score_change_log    — 고득점 변동 로그 (autoincrement ID, analysis_date indexed)
```

**파일** (8개):
```
app/utils/data_loader.py              (관심종목/히스토리/알림 CRUD 함수 추가)
app/utils/charts.py                   (비교 차트 3개 함수 추가)
app/pages/6_🔀_종목비교.py             (신규 — 종목 비교 페이지)
app/pages/5_📋_종목상세.py             (⭐ 버튼 추가)
app/pages/2_🔍_패턴분석.py             (관심종목 컬럼 + 관리 expander)
app/streamlit_app.py                  (저장 관심종목 탭 + 변동 알림 섹션)
scripts/analysis/backtest_runner.py   (--no-tc, --no-short-trend CLI 플래그)
.streamlit/pages.toml                 (종목 비교 메뉴 추가)
```

**테스트**: 276개 (100% 통과, 변경 없음 — 새 기능은 DB/UI 레이어, 기존 분석 로직 무변경)
...
**테스트**: 276개 (100% 통과) — 기존 269 + 신규 7

---

**비교 백테스트 결과** (2025-01-01 ~ 2025-12-31, Long 전략):

| 조건 | 버전 | 총수익률 | MDD | 샤프 | 칼마 | 거래수 | 승률 | 평균승리 | PF |
|------|------|---------|-----|------|------|--------|------|---------|-----|
| min_score=60 | **레거시** | **+24.41%** | -15.67% | **0.93** | 1.54 | 231 | 44.2% | **+11.24%** | **1.30** |
| min_score=60 | **현재** | +22.84% | **-14.54%** | 0.91 | **1.56** | 327 | **48.9%** | +7.36% | 1.26 |
| min_score=70 | **레거시** | +25.89% | -13.81% | **1.19** | 1.86 | 208 | 45.7% | **+11.24%** | **1.37** |
| min_score=70 | **현재** | **+28.15%** | **-12.80%** | 1.16 | **2.18** | 316 | **50.0%** | +7.67% | 1.35 |
...
- **레거시 특성**: 더 적은 거래수(집중), 더 높은 평균 승리폭 → 상승장 포지션 집중

**권장 설정 (현재 스코어링 기준)**:
```
min_score ≥ 70, min_signals ≥ 1
→ 총수익률 +28.15%, MDD -12.80%, 칼마 2.18, 승률 50.0%
```
