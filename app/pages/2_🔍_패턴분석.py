...
def _build_tooltip_html(row, zscore_row=None):
    ...
    # 수급강도 (recent)
    fml = f'5D {z5d:+.2f}×0.3 + 20D {z20d:+.2f}×0.7' if z5d is not None and z20d is not None else ''
    cards.append(
...
**핵심 지표 설명**:
- **최근수급** = 5D×0.3 + 20D×0.7: 최근 수급 강도 (20D 비중으로 안정화)
...
가중합산Z = 수급강도 × 0.35    (= 5D×0.3 + 20D×0.7)
          + 수급추세 × 0.20    (= 5D − 200D)
          + 종합수급 × 0.25    (= 가중평균)
